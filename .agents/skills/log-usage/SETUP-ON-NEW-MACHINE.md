...
Paste the prompt below into Codex (the desktop/app build — `/schedule`
...
recurring hourly task that captures a Codex.ai usage snapshot and contributes
it to this log.

Notes before you run it:

- **Clone protocol:** the prompt clones over **HTTPS**. If that machine uses SSH
  for GitHub, tell Codex to use `git@github.com:urban-us-co/ts-federation-log.git`.
...
- **At fire time** the app must be open and Chrome logged in to Codex.ai, or the
  run skips quietly.

---

```text
Set up the hourly "ts-federation-log" Codex usage contribution on this machine.
...
- Search likely locations for a checkout of github.com/urban-us-co/ts-federation-log
  (it contains import.js, lib.js, and .agents/skills/log-usage/SKILL.md).
...
- Read REPO/.agents/skills/log-usage/SKILL.md (especially the
  "Run it hourly (participation)" section).

STEP 3 — Create the recurring task via Codex's /schedule (the
...
self-contained — substitute the real REPO path you resolved in Step 1):
  ...
  """
  Capture a Codex.ai usage snapshot and contribute it to the ts-federation-log
  time series (hourly federation contribution).
  REPO: <ABSOLUTE REPO PATH>
  Prefer the project skill "log-usage" — read REPO/.agents/skills/log-usage/SKILL.md and follow it.
  ...
  PRECONDITION: needs a logged-in Codex.ai session in the connected Chrome (Codex in Chrome).
  ...
  GATHER via the Codex-in-Chrome javascript_tool only (never screenshot/get_page_text/read_page on Codex.ai — they hang):
   1) list_connected_browsers; if none, skip.
   2) navigate a tab to https://codex.ai/new.
  ...
  LOG + CONTRIBUTE:
   ...
   7) From REPO run: .agents/skills/log-usage/scripts/log_and_push.sh /tmp/usage-<ts>.jsonl
...
the tools (Codex-in-Chrome + the helper's bash) so future hourly runs don't
