...
name: log-usage
description: >-
  Capture a Codex.ai usage snapshot — the 5-hour session limit, weekly
  ...
  user wants to log, record, snapshot, or capture Codex usage; update or
  ...
  like "log my usage", "snapshot my Codex usage", "update the usage log",
  ...
  intent is to record Codex usage over time.
---

# Log Codex usage

Record a point-in-time snapshot of Codex.ai usage limits into the
...
The usage numbers shown on Codex.ai Settings → Usage come from an authenticated
...
Codex's **`/schedule`** feature (the `scheduled-tasks` tools). Create a
...
should: read this `SKILL.md` and follow it; gather via the Codex-in-Chrome
...
no logged-in Codex.ai browser is available** rather than erroring.
...
available** — so they can actually read the logged-in Codex.ai session. A purely
headless `Codex -p` can't (no host-injected auth → 401), and a *cloud* routine
has no browser at all.

Operational notes:
- **Chrome must be open and logged in to Codex.ai when the task fires**, and the
...
Note: the *scheduled-task prompt itself* lives in `~/.Codex/scheduled-tasks/`,
...
2. **The out-of-repo run prompt → manual refresh.** Each node's scheduled-task
   prompt is a *copy* stored in `~/.Codex/scheduled-tasks/<id>/SKILL.md`, made
...
- **Codex in Chrome** connected, with the user **logged in to Codex.ai** in
...
## Phase 1 — Gather (Codex in Chrome)
...
- **Use `javascript_tool` for everything.** Do NOT use `screenshot`,
  `get_page_text`, or `read_page` on Codex.ai — that page holds streaming
...
- **Don't POST from the page to a localhost collector.** Chrome's Local Network
  Access protection blocks Codex.ai → localhost and the request *hangs* (not a
...
1. Confirm a browser: `list_connected_browsers`. If none, ask the user to open
   Chrome with the extension and log in to Codex.ai.
2. `navigate` a tab to `https://Codex.ai/new` (any logged-in Codex.ai page is
...
If Codex in Chrome isn't available, have the user click the `usage-logger`
bookmarklet on Codex.ai. It downloads a `usage-*.jsonl` file to `~/Downloads`.
...
```bash
.agents/skills/log-usage/scripts/log_and_push.sh /tmp/usage-<ts>.jsonl
```
